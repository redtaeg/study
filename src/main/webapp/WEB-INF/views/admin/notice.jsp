  ...
  <div id="myCarousel" class="carousel slide mb-6" data-bs-ride="carousel">
   ...
   <img class="d-block mx-auto mb-4" src="/resources/img/main3.jpg" alt="캐로셀3" width="100%">      
    ...
    <div class="container markerting" style = "min-height:600px;">
		...
		<c:choose>
		<c:when test="${noticeDos == null}">
			<section class="py-5 text-center container">
			 <div class="row py-lg-5">
				<div class="col-lg-6 col-md-8 mx-auto">
					<h1 class="fw-bold">등록된 공지사항이 없습니다.!</h1>
				</div>
			 </div>
			</section>
		</c:when>
...
<c:forEach var="notice" items="${noticeDos}" varStatus="loop"> <!-- 역순 출력 -->
    <c:set var="reverseIndex" value="${noticeDos.size() - loop.index}" />
    <tr>
        <td>${noticeDos[reverseIndex - 1].ez_notice_no}</td>
        
        <td style="width: 500px">
            <c:url value='/admin/notice_detail' var='notice_Info_url'>
                <c:param name='ez_notice_no' value='${noticeDos[reverseIndex - 1].ez_notice_no}'/>
            </c:url>
            <a href="${notice_Info_url}" style="color: black; text-decoration: none;"><strong>${noticeDos[reverseIndex - 1].ez_notice_title}</strong></a>
        </td>
        
        <td>관리자</td>
        
        <td>${noticeDos[reverseIndex - 1].ez_notice_reg_date.substring(0, 10)}</td>
        
        <td>${noticeDos[reverseIndex - 1].ez_notice_views}</td>
    </tr>
</c:forEach>
		      </tbody>
		    </table>
					...
					<c:if test="${loginedTrDo.ez_tr_user_id eq 'super admin'}">
						 <a href="/admin/insert_notice_form" class="btn btn-primary">공지사항 등록</a>
					</c:if>
    </div>
	</main>
	<script src="/resources/css/trainer/checkout.js"></script>
